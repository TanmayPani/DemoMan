import wx; import sys; import pptx; import spacy
from os import path,listdir,makedirs,rename,environ,pathsep
import multiprocessing; import importlib.util
...
import pickle; import shutil; from natsort import natsorted
import whisperx; import subprocess; import numpy as np; import faster_whisper; import ctranslate2
from re import findall,sub; import soundfile as sf; from pathlib import Path
from pptx.util import Pt;from pptx.util import Cm; from pptx.util import Inches
from transformers import Wav2Vec2ForCTC, Wav2Vec2Processor
from kokoro.pipeline import KPipeline; from threading import Thread
...
class Thor(wx.Frame):
    ...
    def __init__(self,parent,id,nlp):
        wx.Frame.__init__(self,parent,id,'Thorlabs Instruction Transcription Interface',size=(1000,600))
        self.nlp = nlp
        ...
        self.CorePath = None; self.CorePathSelector = None; self.StructurePath = None; self.LCB = None
        self.VideoSliceButton=None;self.VideoCombButton=None; self.AudioPath = None; self.VideoPath = None;self.TextPath = None; self.SegPath = None
        self.BOMWriterCB = None ;self.yoffset = 50
        Choices = ['Original','Rewrite']
        self.LCB = wx.StaticText(page1,-1,'Audio Transcription',pos=(40,340+self.yoffset)); self.LCB.Hide()
        self.AudioWriterCB = wx.ComboBox(page1, wx.ID_ANY, choices=Choices, style=wx.CB_READONLY,pos=(40,360+self.yoffset)); self.AudioWriterCB.SetValue('Original')
        self.Bind(wx.EVT_COMBOBOX, self.on_combo_selection, self.AudioWriterCB); self.AudioWriterCB.Hide()
        self.Layout()
        BOMChoices = ['No BOM','BOM']
        # self.LBW = wx.StaticText(page1,-1,'BOM Availability',pos=(40,10 +self.yoffset)); self.LCB.Hide()
        self.BOMWriterCB = wx.ComboBox(page1, wx.ID_ANY, choices=BOMChoices, style=wx.CB_READONLY,pos=(40,10)); self.BOMWriterCB.SetValue('No BOM')
        self.Bind(wx.EVT_COMBOBOX, self.BOMSelection, self.BOMWriterCB)
        # self.Layout()
        
        self.WordSegments = None; self.StepAudio = None; self.StepVideo = None
        self.VideoClips = None; self.ChangeLines = None
        self.VideoSlicerButton = None; self.BKeys=None; self.StepPIDs =None; self.StepTools =None
    
        self.PPTXTitleSlide = None;self.PPTXTitleSlideSubtitle = None
        ...
        new_font = wx.Font(new_font_size,status_font.GetFamily(),status_font.GetStyle(),status_font.GetWeight(),status_font.GetUnderlined(),   status_font.GetFaceName())
        ...
        CPS = wx.Button(page1,label='Core Path',pos=(40,10+self.yoffset),size=(150,-1))
        self.Bind(wx.EVT_BUTTON,self.PathSelector,CPS)
        VCB = wx.Button(page1,label='Video Compressor',pos=(40,50+self.yoffset),size=(150,-1))
        self.Bind(wx.EVT_BUTTON,self.VideoCombination,VCB)
        VSB = wx.Button(page1,label='Video Slicer',pos=(40,90+self.yoffset),size=(150,-1))
#         self.Bind(wx.EVT_BUTTON,self.start_transcription,VSB)
        self.Bind(wx.EVT_BUTTON,self.TranscriptionModel,VSB)
        self.VideoSliceButton = VSB;self.VideoCombButton=VCB
        ...
        self.PPTXName = wx.TextCtrl(page1,pos = (360,10+self.yoffset),size=(200,-1));self.PPTXName.SetValue('Test')
        self.PPTXTitleSlideTitle = wx.TextCtrl(page1,pos = (360,50+self.yoffset),size=(200,-1));self.PPTXTitleSlideTitle.SetValue('Test')
        self.PPTXTitleSlideSubtitle = wx.TextCtrl(page1,pos = (360,90+self.yoffset),size=(200,-1));self.PPTXTitleSlideSubtitle.SetValue('Test')
    ...
    def PathSelector(self, e):
        ...
        fileDialog = wx.DirDialog(frame, "Choose a directory", "",
                 wx.DD_DEFAULT_STYLE | wx.DD_DIR_MUST_EXIST)
        if fileDialog.ShowModal() == wx.ID_OK:
            result = fileDialog.GetPath().replace('\\','/')+'/'
            self.CorePath = result
            Structure = listdir(result)
            self.status = self.SetStatusText('Successfully loaded: '+result+' -- Structure: '+str(Structure))
        else:
            self.status = self.SetStatusText('Command Terminated.')  # Untoggle the button
    #                 print(self.filepath)
        ...
        self.TextPath = (self.CorePath+'StepSegsTxt/');self.VideoPath=(self.CorePath+'Videos/');self.SegPath = (self.CorePath+'StepSegs/');self.AudioPath = (self.CorePath+'StepSegsAudio/')
        self.AudioWriterCB.Show();self.LCB.Show();self.Layout()
    ...
    def FileMover(self):
        ...
        if 'Videos' in self.StructurePath:
            CoreFiles = [i for i in listdir(source_path) if '.MP4'.casefold() in i.casefold()]
        if len(CoreFiles)>0:
        # try:
            # Move the files
            for i in CoreFiles:
                ...
                self.status = self.SetStatusText('Moved files into -- '+ str(destination_path)+str(CoreFiles[0]) +' successfully.')  # Untoggle the button

        else:
            self.status = self.SetStatusText('Nothing to move, or Error moving file into -- '+ str(destination_path)+'. Verify it exists in core directory or if moved earlier.')  # Untoggle the button
    ...
    def VideoCombination(self,e):
        ...
        def work():
            try:
                if 'combined.mp4' not in listdir(VideoPath):
                    VFiles = [i for i in listdir(VideoPath) if '.MP4'.casefold() in i.casefold() if '.jpg' not in i]
                    # print(VFiles)
                    if len(VFiles) >1:
                        wx.CallAfter(self.s.SetLabel, "Multiple videos identified. Initiate sorting sequence.")
        ...
        #                 SortedVFiles = [VideoFileClip(VideoPath+VFiles[SegVidInds[i]]) for i in range(len(SegVidInds))]
                        SortedVFiles = [VideoFileClip(VideoPath+i) for i in SegVidInds]
                        SortedVFiles[0].save_frame(self.CorePath+"StepSegs/"+'FirstFrame.jpg', t=0)
                        wx.CallAfter(self.s.SetLabel, "Sorting complete. Initiate concatenation.")

                        CombinedVid =  concatenate_videoclips(SortedVFiles)
                        wx.CallAfter(self.s.SetLabel, "Concatenation complete. Writing video to output. Please be patient.")

                        CombinedVid.write_videofile(VideoPath+'combined.mp4',temp_audiofile="temp-audio.m4a", remove_temp=True, audio_codec="aac", codec="libx264",threads=8,logger=None,preset='veryfast')
                        ...
                        wx.CallAfter(self.s.SetLabel, "Video writing complete. Now writing audio to output.")

                        CombinedVid.audio.write_audiofile(self.CorePath+'StepSegsAudio/'+"combined.mp3")
                        wx.CallAfter(self.s.SetLabel, "Audio successfully embedded in video. Please proceed to next step.")
...
#                         CombinedVid.audio.write_audiofile(self.CorePath+'StepSegsAudio/'+"wavf_subs.wav", fps=16000, nbytes=2, codec='pcm_s16le', logger=None)
                        ...
                        CombinedVid.close()
                    if len(VFiles) == 1:
                        wx.CallAfter(self.s.SetLabel, "Single video identified. Initiate renaming logic.")
                        rename(VideoPath+VFiles[0],VideoPath+'combined.mp4')
                        video = VideoFileClip(VideoPath+'combined.mp4')
                        video.save_frame(self.CorePath+"StepSegs/"+'FirstFrame.jpg', t=0)
                        wx.CallAfter(self.s.SetLabel, "Audio embedding start.")
                        video.audio.write_audiofile(self.CorePath+'StepSegsAudio/'+'combined.mp3')
                        wx.CallAfter(self.s.SetLabel, "Audio embedding successful. Please proceed to next step")
...
#                         CombinedVid.audio.write_audiofile(self.CorePath+'StepSegsAudio/'+"wavf_subs.wav", fps=16000, nbytes=2, codec='pcm_s16le', logger=None)
                    ...
                    video.save_frame(self.CorePath+"StepSegs/"+'FirstFrame.jpg', t=0)
                    video.close()
                    wx.CallAfter(self.s.SetLabel, 'combined.mp4 already exists. Please delete or move if another instance needs to be created.')
                    ...
                    wx.CallAfter(self.s.SetLabel, 'Rewriting audio of existing combined.mp4 file.')
                    video = VideoFileClip(VideoPath+'combined.mp4')
                    video.audio.write_audiofile(self.CorePath+'StepSegsAudio/'+'combined.mp3')
                    wx.CallAfter(self.s.SetLabel, 'Audio successfully embedded. Please proceed to next step.')
    ...
    def TimeSlicer(self):
        ...
        for i in range(len(word_segments)):
            if i+2 < len(word_segments):
                if 'start' in word_segments[i]['word'].casefold() and 'step' in word_segments[i+1]['word'].casefold():
                    StartSeg = i
                if ('end' in word_segments[i]['word'].casefold() or 'and' in word_segments[i]['word'].casefold() or 'finish' in word_segments[i]['word'].casefold() or 'stop' in word_segments[i]['word'].casefold()) and 'step' in word_segments[i+1]['word'].casefold():
                    EndSeg =i+3
        ...
        CompleteStepTiming = [[Step[i][0]['start'],Step[i][len(Step[i])-1]['end']] for i in range(len(Step))]
        return Step,StichedStep,CompleteStepTiming

    def TranscriptionModel(self,e):
        self.status = self.SetStatusText('Transcription sequence initiated.')  # Untoggle the button
...
#             device='cpu'
        ...
        def work():
            try:
                ...
                PreliminaryTranscription = Model.transcribe(LoadedAudio, batch_size=batch_size, language="en")
                wx.CallAfter(self.s.SetLabel, "Time projection.")
                AlignModel, Metadata = whisperx.load_align_model(language_code="en", device=device)
                AlignedTranscription = whisperx.align(PreliminaryTranscription["segments"], AlignModel, Metadata, AudioFile+'combined.mp3', device, return_char_alignments=False)
                wx.CallAfter(self.s.SetLabel, "Begin writing.")
                self.WordSegments = AlignedTranscription['word_segments']
                wx.CallAfter(self.s.SetLabel, "Alignment sequence complete. Initiate step isolation sequence.")
                self.FullSteps,self.StichedSteps,self.StichedTiming = self.TimeSlicer()
                wx.CallAfter(self.s.SetLabel, "Step Isolation sequence complete. Obtaining audio slices")
                self.StepAudio = [self.AudioWriter(self.StichedSteps[i],self.AudioPath+'AFHeart',i,self.AudioWriterCB.GetValue()) for i in range(len(self.StichedSteps))]
                wx.CallAfter(self.s.SetLabel, "Audio slices obtained. Initiate video rendering.")
                self.StepVideo = [self.VideoStepWriter(self.StichedTiming[i],i) for i in range(len(self.StichedTiming))]
                wx.CallAfter(self.s.SetLabel, "Videos successfully rendered. Presentation creation initiated.")
                if self.BOMWriterCB.GetValue() == 'BOM':
                    wx.CallAfter(self.s.SetLabel, "Extracting BOM data.")
                    self.BOMWriter()
                    wx.CallAfter(self.s.SetLabel, "BOM data extracted. Starting presentation.")
                    self.PresentationGenerator()
                else:
                    wx.CallAfter(self.s.SetLabel, "Starting presentation.")
                    self.NoBOMPresentationGenerator()
                wx.CallAfter(self.s.SetLabel, "KokoroAI Processes Complete. Thank you for your patience!") # Thread-safe update
...
#                 ManualOverride, voice='af_heart', # <= change voice here
        ...
        for i, (gs, ps, audio) in enumerate(generator):
            ...
            sf.write(Tag+'Step'+str(i)+'.mp3', audio, 24000) # save each audio file
    ...
    def VideoStepWriter(self,e,Index):
        ...
        VideoPath = self.VideoPath; AudioPath = self.AudioPath; Timing = self.StichedTiming; VideoClips = []
...
#         print(self.SegPath+"AFHeart"+str(Index)+".mp4")
        MiniVid.write_videofile(self.SegPath+"AFHeart"+str(Index)+".mp4", codec="libx264", audio_codec="aac",preset='veryfast',logger=None,threads=8)
...
#         thread.start()
    def render_comb_video(self,clip, output_path,threads):
        clip.write_videofile(output_path, temp_audiofile="temp-audio.m4a", remove_temp=True, audio_codec="aac", codec="libx264",threads=threads,preset = 'ultrafast',logger=None)     
#         wx.CallAfter(self.on_export_finished)
#     def on_export_finished(self):
#         self.VideoSliceButton.Enable()
#         wx.MessageBox("Export complete!", "Info")
    def BOMWriter(self):
        self.BOMPath = self.CorePath+'BOM/'; BomPath = self.BOMPath
        self.BKeys,self.StepPIDs,self.StepTools = self.standardized_excel_reader(BomPath)
        self.ComponentWriter(self.TextPath,'AFHeartTxt.pkl',[self.StepPIDs,self.StepTools])
    ...
    def standardized_excel_reader(self,fpath):
        ...
        for i in dfs:
            ...
            if len(i['Item number'])>0:
                ...
                StepPIDs.append(ig[0].dropna().reset_index(drop=True).drop(['grouper'],axis=1))
                StepTools.append(ig[1]['Tool Description'].dropna().reset_index(drop=True)[1:])

            count+=1
        return keys,StepPIDs,StepTools
    def PresentationVideoWriter(self,vid,pres,stepindex,addslide):
        left = Cm(33.87/2);top = Cm(0.37);width = Cm(8);height = Cm(6);mime_type = "video/mp4"

        if addslide == True:
            sf = pres.slide_layouts[5] # Get a title only slide layout
            s = pres.slides.add_slide(sf)
            Title1 = s.shapes.title; s.text = 'Step '+ str(stepindex)
            FFPath = self.CorePath+"StepSegs/"+'FirstFrame.jpg'
            movie_shape = s.shapes.add_movie(vid,left,top,width,height,mime_type=mime_type,poster_frame_image=FFPath)
        else:
            FFPath = self.CorePath+"StepSegs/"+'FirstFrame.jpg'
            movie_shape = pres.slides[stepindex].shapes.add_movie(vid,left,top,width,height,mime_type=mime_type,poster_frame_image=FFPath)
...
#         StepData = self.ChangeLines
        ...
        Title1 = slide.shapes.title; Title1.text = 'Assembly Instruction Model'
        subtitle = slide.placeholders[1]; subtitle.text = Subtitle
        sf2 = pres.slide_layouts[5] # Get a title only slide layout


        SegInds = np.argsort(np.concatenate([np.double(findall(r'\d+', i.replace('.mp4','').replace(self.SegPath,'')[-3:])) for i in listdir(self.SegPath) if 'AFHeart' in i]))
        StepVids = listdir(self.SegPath)
        FullStepVidPaths = [self.SegPath+i for i in StepVids  if '.jpg' not in i]
        StichedDF = DataFrame(self.StichedSteps).to_csv(self.TextPath+'Uncorrected_Steps.csv')
        VidParam = False
        offset = 1
        for i in range(len(StepData[0])):
            if len(StepData[0]) == len(FullStepVidPaths):
                ...
                self.PresentationVideoWriter(FullStepVidPaths[SegInds[i]],pres,i+offset,False)
                if self.AudioWriterCB.GetValue() == 'Original':
...
#                     self.ChangeLines = [sub(r" ?\[.*?\]","", item) for item in Changelines]
    ...
    def NoBOMPresentationGenerator(self):
        ...
        title_slide_layout = pres.slide_layouts[0]
        slide = pres.slides.add_slide(title_slide_layout)
        Title1 = slide.shapes.title; Title1.text = MainTitle
        subtitle = slide.placeholders[1]; subtitle.text = Subtitle
...
#             print(SegInds)
            for i in range(len(FullStepVidPaths)):
                print(SegInds[i])
                self.PresentationVideoWriter(FullStepVidPaths[SegInds[i]],pres,i+offset,True)
                self.TextWriter(self.StichedSteps[i],pres,i+offset)
        else:
            data = read_csv(self.TextPath+'Corrected_Files.csv',encoding='latin1')
            Changelines = data['0'].to_list()
#             self.ChangeLines = [sub(r" ?\[.*?\]","", item) for item in Changelines]
    ...
    def TableWriter(self,data,pres,tabletype,stepindex):
        if tabletype == 'Component':
            rows=data.shape[0] + 1; cols=data.shape[1]
    ...
    def TextWriter(self,txt,pres,stepindex):
        slide = pres.slides[stepindex]
        left = Cm(0.5); top = Cm(4); width = Cm(16); height = Cm(2)
        txBox = slide.shapes.add_textbox(left, top, width, height)
        tf = txBox.text_frame; tf.word_wrap = True; tf.text = txt
